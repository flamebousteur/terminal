class terminal {
	constructor(terminalel, row = 10, column = 10) {
	...
	putchar(char) {
		...
		if (char.length > 1)
			throw new Error("putchar: char must be a single character or a number");
		if (this.escape) {
			// the escape sequence
			switch (char) {
				case '[':
	...
	key(e) {
		...
		if (e.key.length === 1) {
			...
			if (this.inpos > 0) {
				...
				if (this.terminal.cursor[0] < 0)
					this.terminal.cursor[0] = 0;
				this.oncursormove();
			}
			...
			if (this.inpos < this.in.length) {
				this.inpos++;
				this.terminal.cursor[0]++;
				if (this.terminal.cursor[0] >= this.terminal.column)
					this.terminal.cursor[0] = this.terminal.column - 1;
				this.oncursormove();
			}
